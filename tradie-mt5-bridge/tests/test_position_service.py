...
class TestGetPositions:
    async def test_returns_list_from_client(self, service, mock_client):
    ...
    async def test_filters_by_symbol(self, service, mock_client):
        mock_client.get_positions.return_value = [_SAMPLE_POSITION]
        positions = await service.get_positions(symbol="EURUSD")
        mock_client.get_positions.assert_called_once_with(symbol="EURUSD")
        assert len(positions) == 1

    async def test_returns_empty_list(self, service, mock_client):
        mock_client.get_positions.return_value = []
        positions = await service.get_positions()
        assert positions == []

    async def test_propagates_connection_error(self, service, mock_client):
...
class TestGetPendingOrders:
    async def test_returns_pending_orders(self, service, mock_client):
...
class TestModifyPosition:
    async def test_modify_sl_tp_success(self, service, mock_client):
        mock_client.modify_position.return_value = OrderResult(success=True, order_id=300001)
        result = await service.modify_position(300001, sl=1.088, tp=1.105)
        assert result.success is True
        mock_client.modify_position.assert_called_once_with(ticket=300001, sl=1.088, tp=1.105)

    async def test_modify_only_sl(self, service, mock_client):
        mock_client.modify_position.return_value = OrderResult(success=True, order_id=300001)
        result = await service.modify_position(300001, sl=1.088)
        assert result.success is True
        mock_client.modify_position.assert_called_once_with(ticket=300001, sl=1.088, tp=None)

    async def test_modify_failure_returned(self, service, mock_client):
        mock_client.modify_position.return_value = OrderResult(
            success=False, error_message="Position 999 not found"
        )
        result = await service.modify_position(999, sl=1.085)
        assert result.success is False
        assert "not found" in result.error_message

    async def test_connection_error_raises(self, service, mock_client):
        ...
        with pytest.raises(ConnectionError, match="MT5 disconnected"):
            await service.modify_position(300001, sl=1.088)


class TestClosePosition:
    async def test_close_full_position(self, service, mock_client):
        mock_client.close_position.return_value = OrderResult(
            success=True, deal_id=500001, volume=0.1, price=1.09200
        )
        result = await service.close_position(300001)
        assert result.success is True
        assert result.deal_id == 500001
        mock_client.close_position.assert_called_once_with(ticket=300001, volume=None)

    async def test_close_partial_position(self, service, mock_client):
        mock_client.close_position.return_value = OrderResult(
            success=True, deal_id=500002, volume=0.05, price=1.09200
        )
        result = await service.close_position(300001, volume=0.05)
        assert result.success is True
        mock_client.close_position.assert_called_once_with(ticket=300001, volume=0.05)

    async def test_close_position_failure_returned(self, service, mock_client):
        mock_client.close_position.return_value = OrderResult(
            success=False, error_message="Position 999 not found"
        )
        result = await service.close_position(999)
        assert result.success is False

    async def test_connection_error_raises(self, service, mock_client):
        mock_client.close_position.side_effect = ConnectionError("MT5 disconnected")
        with pytest.raises(ConnectionError, match="MT5 disconnected"):
            await service.close_position(300001)


class TestCloseAllPositions:
    async def test_close_all_success(self, service, mock_client):
        ...
        results = await service.close_all_positions()
        assert len(results) == 2
        assert all(r.success for r in results)
        mock_client.close_all_positions.assert_called_once_with(symbol=None)

    async def test_close_all_by_symbol(self, service, mock_client):
        mock_client.close_all_positions.return_value = [
            OrderResult(success=True, deal_id=500001, volume=0.1, price=1.09200),
        ]
        results = await service.close_all_positions(symbol="EURUSD")
        mock_client.close_all_positions.assert_called_once_with(symbol="EURUSD")
        assert len(results) == 1

    async def test_close_all_returns_empty_when_no_positions(self, service, mock_client):
        mock_client.close_all_positions.return_value = []
        results = await service.close_all_positions()
        assert results == []
    ...
    async def test_partial_close_failure_reported(self, service, mock_client):
        mock_client.close_all_positions.return_value = [
