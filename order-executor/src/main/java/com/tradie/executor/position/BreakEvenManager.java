...
/**
 * Moves the stop loss to break-even once a position reaches the configured profit threshold.
 * Tracks activated positions in memory to avoid re-processing.
 */
@Service
public class BreakEvenManager {

    private static final Logger log = LoggerFactory.getLogger(BreakEvenManager.class);
    ...
    // positionIds where break-even has already been activated
    private final Set<UUID> activated = ConcurrentHashMap.newKeySet();

    // Lazy to break circular dependency: IBConnectionManager → MarketDataService → BreakEvenManager → IBConnectionManager
    ...
    /**
     * Checks whether break-even stop should be activated for this position.
     * Only activates once per position lifetime.
     */
    public void checkAndActivate(Position position, BigDecimal currentPrice) {
        PositionProperties.BreakEven config = positionProperties.getBreakEven();
        if (!config.isEnabled()) return;
        if (position.getStatus() != Position.PositionStatus.OPEN) return;
        if (activated.contains(position.getId())) return;
        ...
        activated.add(position.getId());

        modifyStopOnIbkr(position, breakEvenPrice);
        log.info("Break-even activated: symbol={} breakEvenPrice={} positionId={}",
                position.getSymbol(), breakEvenPrice, position.getId());
    ...
    private void modifyStopOnIbkr(Position position, BigDecimal newStopPrice) {
    ...
    public void clearActivated(UUID positionId) {
        activated.remove(positionId);
    }
}
